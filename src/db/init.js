...
async function performFirstTimeSetup(db) {
  ...
  await db.exec("CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY AUTOINCREMENT, mailbox_id INTEGER NOT NULL, sender TEXT NOT NULL, to_addrs TEXT NOT NULL DEFAULT '', subject TEXT NOT NULL, verification_code TEXT, preview TEXT, text_content TEXT DEFAULT '', html_content TEXT DEFAULT '', received_at TEXT DEFAULT CURRENT_TIMESTAMP, is_read INTEGER DEFAULT 0, FOREIGN KEY(mailbox_id) REFERENCES mailboxes(id));");
 ...
 * 检查并添加缺失的字段：text_content, html_content
 * @param {object} db - 数据库连接对象
 * @returns {Promise<void>}
 */
async function migrateMessagesFields(db) {
  try {
    ...
    // 添加 text_content 字段（纯文本内容）
    if (!columnNames.includes('text_content')) {
      await db.exec("ALTER TABLE messages ADD COLUMN text_content TEXT DEFAULT '';");
      console.log('已添加 messages.text_content 字段');
    }
    
    // 添加 html_content 字段（HTML内容）
    if (!columnNames.includes('html_content')) {
      await db.exec("ALTER TABLE messages ADD COLUMN html_content TEXT DEFAULT '';");
